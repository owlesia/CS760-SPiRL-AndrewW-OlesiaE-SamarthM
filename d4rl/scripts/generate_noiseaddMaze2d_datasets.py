...
def main():
    ...
    parser.add_argument('--noise', type=float, default=float(0), help='degree of noise to add. 0 = no noise, 1 = all noise')
    ...
    for tt in tqdm.tqdm(range(args.num_samples)):
        ...
        noise = args.noise
        if noise != 0:
            act += noise*np.random.randn(*act.shape)
...
def save_data(args, data, idx):
    ...
    dir_name = 'maze2d-additive-%s-noisy' % args.noise
    if args.batch_idx >= 0:
